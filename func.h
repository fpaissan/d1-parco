...
// #pragma simd
// #pragma ivdep // Hlong: ignore dependencies for vectorization
#pragma GCC unroll 2 // Suggest unrolling for improved performance
        for (long j = 0; j < i; j=j+2) {
...
void matTranspose_CO(float* matrix, float* result,
    ...
    long row_size = row_end - row_start;
    long col_size = col_end - col_start;
    ...
    // Recursive case: split longo quadrants
    long row_mid = (row_start + row_end) / 2;
    long col_mid = (col_start + col_end) / 2;
...
void matTransposeTiled(float* matrix, float* result,
                       const long size, const long tile_size) {
    // First process diagonal tiles
    for (long i = 0; i < size; i += tile_size) {
        // Process diagonal tile; separate to avoid conflict with other tiles
        for (long ti = i; ti < i + tile_size && ti < size; ti++) {
            for (long tj = i; tj < i + tile_size && tj < size; tj++) {
                result[tj * size + ti] = matrix[ti * size + tj];
            }
        }

        // Process pairs of off-diagonal tiles
        for (long j = i + tile_size; j < size; j += tile_size) {
            // Process tile (i,j) and (j,i) in parallel to maximize cache usage
            for (long ti = i; ti < i + tile_size && ti < size; ti++) {
                for (long tj = j; tj < j + tile_size && tj < size; tj++) {
